...
fn scanRoot(
    ...
    const direct_file = try std.fs.path.join(allocator, &.{ root, "achievements.json" });
    ...
    while (try iterator.next(io)) |entry| {
        ...
        const state_file = try std.fs.path.join(allocator, &.{ root, entry.name, "achievements.json" });
...
test "empty or missing roots produce no candidates" {
    var result = try discover(std.testing.allocator, std.testing.io, &.{"this-path-does-not-exist"});
    defer result.deinit();
